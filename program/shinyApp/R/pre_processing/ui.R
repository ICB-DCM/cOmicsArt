  ...
  div(
    ...
    fluidRow(
      column(
        6,
        h4("Raw Data"), #
        plotOutput("raw_violin_plot") # Target ID for copy: raw_violin_plot
      ),
      column(
        6,
        h4("Pre-processed Data"), #
        plotOutput("preprocessed_violin_plot") # Target ID for copy: preprocessed_violin_plot
      )
    ),
    ...
    fluidRow(column(4, "", 
      ...
      4, 
      h5("Violin Plot Actions"), # Modified for clarity
      actionButton(
    ...
    splitLayout(
      ...
      div(textAreaInput(
        ...
        label = "Notes:", #
        placeholder = NOTES_PlACEHOLDER, # Ensure NOTES_PlACEHOLDER is defined globally or sourced
        width = "1000px" #
      ) %>% helper(type = "markdown", content = "TakingNotesMD_help"), #
      helpText(NOTES_HELP)), # Ensure NOTES_HELP is defined globally or sourced
      NULL
    )
  )
)
