entitieSelection=function(data,
  ...
  if(!(is.na(additionalInput_sample_annotation_types)) & !(is.na(additionalInput_ctrl_idx)) & !(is.na(additionalInput_cmp_idx))){
    if(any(type=="significant_LFC")){
      ...
      ctrl_samples_idx<-which(data$sample_table[,additionalInput_sample_annotation_types]%in%additionalInput_ctrl_idx)
      comparison_samples_idx<-which(data$sample_table[,additionalInput_sample_annotation_types]%in%additionalInput_cmp_idx)
    ...
    if(any(type=="LFC_onlySig")){
     ctrl_samples_idx<-which(data$sample_table[,additionalInput_sample_annotation_types]%in%additionalInput_ctrl_idx)
     comparison_samples_idx<-which(data$sample_table[,additionalInput_sample_annotation_types]%in%additionalInput_cmp_idx)
     LFC_output=getLFC(data$Matrix,ctrl_samples_idx,comparison_samples_idx)
