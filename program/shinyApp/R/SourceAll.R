source("R/version.R",local = T)
source("R/util.R", local = T)
source("R/C.R",local = T)
# Source constants
source("R/constants/ui_styles.R", local = T)
# Source utility helpers
source("R/utils/plotly_helpers.R", local = T)
source("R/utils/ui_components.R", local = T)
source("R/fun_filterRNA.R",local = T)
source("R/fun_LFC.R",local = T)
source("R/heatmap/fun_entitieSelection.R",local = T)
source("R/fun_LogIt.R",local = T)
source("R/fun_readInSampleTable.R",local = T)
source("R/module_DownloadReport.R",local = T)
source("R/enrichment_analysis/enrichment_analysis.R", local = T)
source("R/enrichment_analysis/overrepresentation_analysis.R", local = T)
source("R/enrichment_analysis/check_annotation.R", local = T)
source("R/enrichment_analysis/translation.R", local = T)
source("R/enrichment_analysis/util.R", local = T)
source("R/enrichment_analysis/server.R", local = T)
source("R/heatmap/server.R",local = T)
source("R/heatmap/util.R",local = T)
source("R/pca/server.R", local = T)
source("R/pca/util.R", local = T)
source("R/pca/plots.R", local = T)
source("R/single_gene_visualisation/server.R",local = T)
source("R/sample_correlation/server.R", local = T)
source("R/sample_correlation/util.R", local = T)
source("R/significance_analysis/server.R", local = T)
source("R/significance_analysis/util.R", local = T)
source("R/significance_analysis/tab_generator.R", local = T)
source("R/pre_processing/util.R", local = T)
source("R/single_gene_visualisation/util.R",local = T)
source("R/ml_classification/ui.R", local = T)
source("R/ml_classification/server.R", local = T)
source("R/ml_classification/util.R", local = T)
source("R/ml_classification/plots.R", local = T)
source("R/ml_classification/methods/kmeans.R", local = T)
source("R/ml_classification/methods/svm.R", local = T)
# Source for R code
source("R/code_generation/function_infos.R",local = T)
source("R/code_generation/util.R",local = T)
source("R/code_generation/plot_generators.R",local = T)
# Source for Report Generation
source("R/report_generation/util.R",local = T)
