server <- function(input,output,session){
  ...
  observeEvent(input$plot_copied_status, {
    status_info <- input$plot_copied_status
    
    # Extract plot ID and status from the status info
    parts <- strsplit(status_info, "_")[[1]]
    plot_id <- parts[1]
    status <- parts[2]
    ...
    if(status == "success") {
      showNotification(
        paste("Plot successfully copied to clipboard!"), 
        type = "message",
        duration = 3
      )
      ...
      error_message <- switch(error_type,
                              "nosrc" = "Could not find image source.",
                              "clipboard" = "Clipboard operation failed.",
                              "fetch" = "Failed to fetch image data.",
                              "unavailable" = "Clipboard API not available in your browser.",
                              "Unknown error")
      
      showNotification(
        paste("Failed to copy plot:", error_message), 
        type = "error",
        duration = 5
      )
    }
