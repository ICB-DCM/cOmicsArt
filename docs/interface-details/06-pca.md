...
- **PCA Interpretation**: Observing the PCA plot can give insights into the relationships between different samples based on the selected annotation and the principal components chosen for the axes. For more information on PCA interpretation, check out [this video on PCA interpretation](https://www.youtube.com/watch?v=FgakZw6K1QQ). A common analysis worklfow would be to pre-process the entire dataset and visualise it within the PCA plot. Coloring after your supplied metadata may help you to idnetify if you have any batch effects - large variation within your data that is not due to the biological question you are asking. You may want to save this plot e.g. within the Report for future comparison or simply reference. If you see such effects and want to remove them you might want to go back to pre-processing and select batch correction for the varaible introducing the unwanted variation. If you assume it is a combination of varaibles which might not be within your initially loaded sample table you will need to adjust the sample table re-upload to cOmicsArt (ensure to really click Upload new data) before you can select the new variable as batch. Note that we use ComBat in the background which can deal with complex batches meaning which might not be necassarly linear. You can go back to the PCA, ensure its update by clicking 'Get PCA' and see the differences.
